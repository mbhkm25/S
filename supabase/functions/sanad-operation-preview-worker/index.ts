...
async function gotenberg(route: string, form: FormData, timeout = 30000) {
  const response = await fetch(
    `${env("GOTENBERG_URL").replace(/\/$/, "")}${route}`,
    {
      method: "POST",
  ...
  if (!response.ok) {
    throw new Error(
      `gotenberg_failed_${response.status}_${
...
async function pngToWebp(png: Uint8Array, width: number, height: number) {
  const html =
    `<!doctype html><html><head><meta charset="utf-8"><style>html,body{margin:0;width:${width}px;height:${height}px;overflow:hidden;background:#fff}img{display:block;width:${width}px;height:${height}px}</style></head><body><img id="source" src="source.png"><script>source.onload=()=>window.__READY__=true</script></body></html>`;
  const form = new FormData();
  form.append(
    "files",
...
function hasContent(
  ...
  return a > 20 &&
    (luminance(r, g, b) < 238 || Math.max(r, g, b) - Math.min(r, g, b) > 16);
}
...
function detectWhitePage(image: Image): Bounds {
  ...
  for (let y = 0; y < image.height; y += step) {
    for (let x = 0; x < image.width; x += step) {
      ...
      if (
        image.bitmap[i + 3] > 20 &&
        luminance(image.bitmap[i], image.bitmap[i + 1], image.bitmap[i + 2]) >
          210
      ) {
        x0 = Math.min(x0, x);
        y0 = Math.min(y0, y);
...
function detectReceiptCrop(image: Image) {
  ...
  let x0 = page.x + page.width - 1,
    y0 = page.y + page.height - 1,
    x1 = page.x,
    y1 = page.y,
    count = 0;
  ...
  const enough =
    count >= Math.max(180, Math.round(page.width * page.height / 18000));
  ...
  const layoutClass = widthRatio >= 0.72
    ? "full_width_receipt"
    : "centered_receipt";
  ...
  let x = layoutClass === "full_width_receipt"
    ? page.x
    : Math.max(page.x, x0 - horizontalPad);
  let right = layoutClass === "full_width_receipt"
    ? page.x + page.width - 1
    : Math.min(page.x + page.width - 1, x1 + horizontalPad);
  let y = Math.max(page.y, y0 - verticalPad);
  let bottom = Math.min(
    page.y + page.height - 1,
    y1 + verticalPad + Math.round(page.height * 0.035),
  );
  ...
  for (let px = x; px < x + width; px += Math.max(5, step * 2)) {
    for (const py of [y + probe, y + height - 1 - probe]) {
      probes += 1;
      if (hasContent(image.bitmap, image.width, px, py)) edgeHits += 1;
    }
  }
  for (let py = y; py < y + height; py += Math.max(5, step * 2)) {
    for (const px of [x + probe, x + width - 1 - probe]) {
      probes += 1;
      if (hasContent(image.bitmap, image.width, px, py)) edgeHits += 1;
    }
  }
  ...
  return {
    ...
    page,
    mode: edgeSafetyPassed
      ? "full-content-union"
      : "full-page-edge-safe-fallback",
    layoutClass,
...
async function buildPdfPreview(sourceUrl: string): Promise<PreviewResult> {
  ...
  const bytes = await pngToWebp(
    await image.encode(3),
    image.width,
    image.height,
  );
  return {
...
async function buildImagePreview(
  ...
  if (mime === "image/webp" && isWebp(source)) {
    const d = webpDimensions(source);
    if (!d) throw new Error("source_webp_dimensions_unknown");
    return {
      bytes: source,
      width: d.width,
      height: d.height,
...
function classifyFailure(message: string): {
  ...
  const statusMatch = message.match(/(?:failed|http)_([45]\d\d)(?:_|$)/i);
  const httpStatus = statusMatch ? Number(statusMatch[1]) : null;
  const permanent =
    /unsupported_mime|invalid_png|dimensions_unknown|decode|encoding_stage_invalid/i
      .test(message);
  return {
    retryable: !permanent &&
      (httpStatus === null || [408, 409, 425, 429].includes(httpStatus) ||
        httpStatus >= 500),
    code: message.split(/[:\s]/, 1)[0].slice(0, 120) ||
...
async function processJob(
  ...
  try {
    ...
    const version = (job.source_sha256 || Date.now().toString()).replace(
      /[^a-zA-Z0-9_-]/g,
      "",
    ).slice(0, 64) || "source";
    const bucket = job.source_bucket || "operation-files";
    const path =
      `previews/operations/${job.operation_id}/${version}-${PIPELINE_VERSION}.webp`;
...
async function requestDrain(service: ServiceClient) {
