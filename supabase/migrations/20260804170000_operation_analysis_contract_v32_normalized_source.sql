-- Extraction v3.2 stores the reconciled party/identifier result under
-- raw_ai_json.normalized. The compatibility trigger must consume that source
-- before the unreconciled Gemini payload, otherwise legacy scalar columns can
-- overwrite the selected financial account with an identity/card number.
...
begin
  ...
  new.structured_data := coalesce(new.structured_data, '{}'::jsonb) || jsonb_build_object(
    ...
    'transaction_time_present', coalesce(
      case
        when lower(coalesce(v_extracted->>'transaction_time_present', '')) in ('true','1','yes') then true
        when lower(coalesce(v_extracted->>'transaction_time_present', '')) in ('false','0','no') then false
      end,
      new.transaction_time_present
    ),
    'transaction_date_source', coalesce(
      nullif(trim(v_extracted->>'transaction_date_source'), ''),
      new.transaction_date_source
    )
  );
