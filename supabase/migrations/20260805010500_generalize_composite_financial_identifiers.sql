...
alter table public.business_financial_identifiers
  ...
  check (identifier_type = any (array[
    'account_number'::text,'wallet_number'::text,'customer_line'::text,
    'merchant_point'::text,'terminal_number'::text,'phone_number'::text,
    'national_id'::text,'passport_number'::text,'unique_account_name'::text,
    'iban'::text,'other'::text
  ]));
...
begin
  ...
  perform 1 from public.business_profiles bp
  ...
  select exists(
    select 1 from jsonb_array_elements(p_identifiers) item
  ...
  if v_account_id is not null then
    ...
    where a.id=v_account_id and a.business_id=p_business_id for update;
  ...
  on conflict(id) do update set
    ...
    verification_status='unverified',verified_at=null,verified_by_user_id=null,
    status='active',metadata=public.business_financial_accounts.metadata||excluded.metadata,
    updated_at=now();
  ...
  loop
    ...
    insert into public.business_financial_identifiers(
      ...
      'unverified','active',jsonb_build_object(
  ...
  select entry.value into v_item from jsonb_array_elements(v_items) entry(value)
...
begin
  ...
  v_identifier_type:=case
    when v_identifier_type='financial_account_number' then 'account_number'
  ...
  with candidates as(
    ...
    where fi.status='active' and fi.routing_enabled=true
      ...
      and (
        ...
        or (v_identifier_type not in ('phone_number','unique_account_name')
          and fi.identifier_type=v_identifier_type
  ...
  if v_account_verified='verified' and v_identifier_verified='verified' then
    ...
    insert into public.business_payment_inbox(
      ...
      v_match_strategy||'_unverified',jsonb_build_object(
        'identifier',case when v_identifier_type='phone_number' then v_phone else v_identifier end,
        'identifier_type',v_identifier_type,'identifier_source',v_identifier_source,
        'receiver_name_normalized',case when v_composite then v_holder_name else null end,
