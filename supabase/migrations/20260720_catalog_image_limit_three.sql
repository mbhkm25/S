-- Keep catalog media lightweight on mobile: maximum three images per item.
...
begin
  ...
  if jsonb_array_length(coalesce(p_image_paths,'[]'::jsonb)) > 3 then raise exception 'catalog_image_limit_exceeded'; end if;
  ...
  if p_status='active' then
    select count(*) into v_active_count from public.business_catalog_items where business_id=p_business_id and status='active' and (p_item_id is null or id<>p_item_id);
    if v_active_count>=10 then raise exception 'active_catalog_limit_reached'; end if;
  end if;
  if p_item_id is null then
    insert into public.business_catalog_items(business_id,created_by_user_id,item_type,title,description,price,currency,image_paths,features,status,display_order,is_featured,availability_status,contact_action)
    values (p_business_id,v_user_id,p_item_type,trim(p_title),nullif(trim(coalesce(p_description,'')),''),p_price,p_currency,coalesce(p_image_paths,'[]'::jsonb),coalesce(p_features,'[]'::jsonb),p_status,coalesce(p_display_order,100),coalesce(p_is_featured,false),p_availability_status,p_contact_action)
    returning * into v_item;
  else
    update public.business_catalog_items set item_type=p_item_type,title=trim(p_title),description=nullif(trim(coalesce(p_description,'')),''),price=p_price,currency=p_currency,image_paths=coalesce(p_image_paths,image_paths),features=coalesce(p_features,features),status=p_status,display_order=coalesce(p_display_order,display_order),is_featured=coalesce(p_is_featured,false),availability_status=p_availability_status,contact_action=p_contact_action,updated_at=now()
