...
begin
  ...
  loop
    perform private.create_notification(
      v_user_id,
      'payment_inbox_review_required',
      'business',
      'warning',
      'دفعة تحتاج مراجعة في وارد المدفوعات',
      concat('طابقت سند رقم الحساب مع ', v_business.name, '، لكن الحساب لم يُوثق بعد. راجع العملية قبل اعتمادها.'),
      'operation_details',
      jsonb_build_object('operation_id',v_operation.id,'public_token',v_operation.public_token,'payment_inbox_id',v_item.id,'business_id',v_item.business_id,'source','notification'),
      null,
      v_item.business_id,
      v_item.operation_id,
      'business_payment_inbox_review',
      v_item.id::text,
      concat('payment_inbox_review_required:',v_item.id,':',v_user_id),
      jsonb_build_object('payment_inbox_id',v_item.id,'source_mode',v_item.source_mode,'match_score',v_item.match_score,'public_token',v_operation.public_token),
...
begin
  ...
  select count(distinct fa.business_id), min(fa.business_id), min(fa.id), min(fa.verification_status), min(fi.verification_status)
  ...
  if v_candidate_count <> 1 or v_business_id is null then
    return jsonb_build_object('ok',false,'reason',case when v_candidate_count=0 then 'exact_identifier_not_found' else 'exact_identifier_ambiguous' end,'candidate_count',v_candidate_count);
  end if;
  ...
  if v_account_verified = 'verified' and v_identifier_verified = 'verified' then
    v_inbox_id := private.enqueue_business_payment_inbox_system(
      v_business_id,p_operation_id,null,v_account_id,'live',100,'exact_credited_identifier',
    ...
    insert into public.business_payment_inbox(
      ...
      v_business_id,p_operation_id,v_account_id,'canary','review_required',95,100,'exact_credited_identifier_unverified',
      jsonb_build_object('identifier',v_identifier,'entity_code',v_operation.financial_entity_code,'entity_mismatch_warning',true,'routing_precedence','exact_identifier_first','verification_required',true)
...
create trigger operations_exact_identifier_routing
after insert or update of ai_status,credited_account_normalized,receiver_account_normalized,document_account_normalized,currency
on public.operations
for each row execute function private.route_operation_by_exact_identifier_trigger();

-- Backfill recent analyzed operations only when no inbox projection exists.
do $$
declare r record;
begin
  for r in
    ...
    where o.ai_status='completed'
      ...
      and not exists(select 1 from public.business_payment_inbox i where i.operation_id=o.id)
  loop
    perform private.route_operation_by_exact_identifier(r.id);
  end loop;
end;
$$;
