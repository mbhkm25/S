...
begin
  select * into v_requested from public.operations where id=p_operation_id;
  ...
  where operation_id=p_operation_id and identity_version=1 and match_type='exact_duplicate'
    and canonical_operation_id is not null and canonical_operation_id<>p_operation_id
  order by evaluated_at desc,id desc limit 1;

  if v_run.id is not null then
    select * into v_canonical from public.operations where id=v_run.canonical_operation_id;
  ...
  if not v_is_exact then
    ...
    from private.operation_submissions s where s.submitted_operation_id=v_requested.id;
  end if;

  return jsonb_build_object(
    'ok',true,'is_exact_duplicate',v_is_exact,
    'match_type',case when v_is_exact then 'exact_duplicate' else coalesce(v_requested.identity_status,'pending') end,
    'requested_operation_id',v_requested.id,'requested_public_token',v_requested.public_token,
    'canonical_operation_id',v_canonical.id,'canonical_public_token',v_canonical.public_token,
    'canonical_token_status',v_canonical.token_status,'canonical_token_expires_at',v_canonical.token_expires_at,
    'canonical_available',(v_canonical.id is not null and coalesce(v_canonical.token_status,'active')='active' and (v_canonical.token_expires_at is null or v_canonical.token_expires_at>now())),
    'first_registered_at',v_canonical.created_at,'occurrence_count',v_occurrence_count,
...
declare
  v_user uuid := auth.uid(); v_operation public.operations%rowtype; v_reuse jsonb; v_access_operation_id uuid;
  v_existing boolean := false; v_usage jsonb;
begin
  if v_user is null then return jsonb_build_object('allowed',false,'reason','not_authenticated','requires_auth',true,'will_consume',false); end if;
  select * into v_operation from public.operations where public_token=p_public_token limit 1;
  if v_operation.id is null then return jsonb_build_object('allowed',false,'reason','operation_not_found','requires_auth',false,'will_consume',false); end if;
  if v_operation.token_status is not null and v_operation.token_status<>'active' then return jsonb_build_object('allowed',false,'reason','token_not_active','requires_auth',false,'will_consume',false); end if;
  if v_operation.token_expires_at is not null and v_operation.token_expires_at<=now() then return jsonb_build_object('allowed',false,'reason','token_expired','requires_auth',false,'will_consume',false); end if;
  if not public.sanad_user_has_basic_profile(v_user) then return jsonb_build_object('allowed',false,'reason','profile_incomplete','requires_profile',true,'will_consume',false); end if;
  ...
  select exists(select 1 from public.operation_access_logs l where l.user_id=v_user and l.operation_id=v_access_operation_id) into v_existing;
  v_usage := public.get_my_operation_access_usage();

  if v_existing then
    return jsonb_build_object('allowed',true,'reason',case when coalesce((v_reuse->>'is_exact_duplicate')::boolean,false) then 'duplicate_previously_opened' else 'previously_opened' end,
      'will_consume',false,'usage',v_usage,'reuse',v_reuse || jsonb_build_object('access_will_consume',false));
  end if;
  if coalesce((v_usage->>'remaining')::integer,0)<=0 then
    return jsonb_build_object('allowed',false,'reason','access_limit_reached','requires_subscription',true,'will_consume',false,'usage',v_usage,
      'reuse',v_reuse || jsonb_build_object('access_will_consume',false));
  end if;
  return jsonb_build_object('allowed',true,'reason',case when coalesce((v_reuse->>'is_exact_duplicate')::boolean,false) then 'duplicate_access_available' else 'new_access_available' end,
    'will_consume',true,'usage',v_usage,'reuse',v_reuse || jsonb_build_object('access_will_consume',true));
end;
$$;
...
begin
  ...
  select coalesce(jsonb_agg(jsonb_build_object(
    'operation_id',i.operation_id,'inbox_id',i.id,'is_exact_duplicate',true,
    'canonical_operation_id',c.id,'canonical_public_token',c.public_token,'first_registered_at',c.created_at,
    'occurrence_count',coalesce(occ.n,1),'canonical_inbox_id',ci.id,'canonical_inbox_status',ci.status,
    'canonical_claimed_by_name',cp.full_name,'canonical_completed_by_name',ep.full_name,'canonical_completed_at',ci.completed_at,
    'strategy',r.evidence->>'strategy','confidence',r.confidence
  ) order by i.created_at desc),'[]'::jsonb) into v_items
  from public.business_payment_inbox i
  join private.operation_identity_shadow_runs r on r.operation_id=i.operation_id and r.identity_version=1 and r.match_type='exact_duplicate'
    and r.canonical_operation_id is not null and r.canonical_operation_id<>i.operation_id
  join public.operations c on c.id=r.canonical_operation_id
  left join public.business_payment_inbox ci on ci.business_id=i.business_id and ci.operation_id=c.id
  left join public.profiles cp on cp.id=ci.claimed_by_user_id
  left join public.profiles ep on ep.id=ci.completed_by_user_id
  left join lateral (select count(*) n from private.operation_submissions s where s.submitted_operation_id=c.id or (s.canonical_operation_id=c.id and s.identity_match_type='exact_duplicate')) occ on true
...
begin
  ...
  if coalesce((v_reuse->>'is_exact_duplicate')::boolean,false) then
    raise exception 'duplicate_operation_requires_canonical' using errcode='P0001', detail=coalesce(v_reuse->>'canonical_operation_id','');
  end if;
  return new;
end;
$$;
...
create trigger business_payment_inbox_duplicate_guard before update of status on public.business_payment_inbox
for each row execute function private.guard_duplicate_payment_inbox_transition();

comment on function private.operation_reuse_resolution(uuid) is 'Canonical reuse resolver. Only exact_duplicate is actionable; probable_duplicate is never redirected or blocked.';
comment on function public.get_business_payment_reuse_notices(uuid) is 'Permission-filtered exact duplicate mapping for Payment Inbox UI. Does not expose probable matches.';
