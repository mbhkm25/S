...
declare
  ...
  v_source text := case
    when lower(trim(coalesce(p_source, ''))) in (
      'operation_details_runtime_v2',
      'operation_details_runtime_v3',
      'operation_details_runtime_v4'
...
comment on function public.complete_operation_workflow(uuid,uuid,uuid,uuid,text,text)
is 'Completes an operation workflow and normalizes legacy operation-details runtime source names to operation_details.';
