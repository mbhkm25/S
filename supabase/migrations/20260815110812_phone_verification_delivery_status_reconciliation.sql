...
begin
  ...
  -- Delivery/read receipts are telemetry only: keep the claim actionable as "sent"
  -- so a later signed WhatsApp button can still complete verification.
  if v_status in ('sent','delivered','read') then
    ...
    -- Preserve verified/rejected/conflict terminal states if a delayed callback arrives.
    if v_claim.status in ('sent','sending') then
      update public.phone_verification_claims
      set status = 'failed',
          claimed_at = null,
          last_error = left(coalesce(nullif(trim(p_error), ''), 'whatsapp_delivery_failed'), 1000),
      ...
      -- Retry only while the same claim is still live and under its attempt cap.
  ...
  if not exists (
    ...
    insert into public.phone_verification_events(
      claim_id, user_id, event_type, external_message_id, metadata, occurred_at
    ) values (
      v_claim.id,
      v_claim.user_id,
      v_event_type,
...
revoke all on function public.apply_phone_verification_whatsapp_delivery_status(text,text,timestamptz,text) from public, anon, authenticated;
grant execute on function public.apply_phone_verification_whatsapp_delivery_status(text,text,timestamptz,text) to service_role;
