...
begin
  ...
  if tg_op = 'INSERT' then
    ...
    v_body := 'تم استلام طلبك وسيُجهّز التقرير، وستظهر حالة تسليمه عبر واتساب بشكل مستقل.';
  elsif new.status is distinct from old.status and new.status in ('ready', 'sent', 'completed') then
    v_type := 'report_ready';
    v_title := 'تقريرك جاهز';
    v_body := 'تم تجهيز التقرير بنجاح. يمكنك فتحه من قسم التقارير، وتظهر حالة إرساله عبر واتساب بشكل مستقل.';
    v_severity := 'success';
    ...
    v_body := 'تعذر إنشاء التقرير. يمكنك إعادة المحاولة من قسم التقارير.';
    v_severity := 'warning';
  else
    return new;
  end if;
