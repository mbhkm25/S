...
create index if not exists idx_operation_pipeline_watchdog_runs_checked_at on private.operation_pipeline_watchdog_runs(checked_at desc);
...
begin
  ...
  select count(*) filter (where j.status='failed')::integer,
  ...
  select count(*)::integer,
         percentile_cont(0.95) within group (order by extract(epoch from (o.analysis_completed_at-o.created_at))*1000)
  into v_sample, v_p95
  from public.operations o
  ...
  insert into private.operation_pipeline_watchdog_runs(
    checked_at,stuck_operations,failed_jobs,dead_letter_jobs,sample_operations,p95_ms,p95_breached,metadata
  ) values (
    v_now,v_stuck,v_failed,v_dead,v_sample,
    case when v_p95 is null then null else round(v_p95::numeric,0) end,
    v_p95_breached,
    jsonb_build_object('stuck_threshold_ms',15000,'p95_target_ms',6000,'window_minutes',60)
  );
  ...
  for v_operation in
    ...
    where o.created_at >= v_now - interval '24 hours'
      ...
      and o.created_at <= v_now - interval '15 seconds'
  loop
    for v_admin in select p.id from public.profiles p where p.global_role='platform_admin' and p.status='active'
    loop
      insert into public.notifications(
        recipient_user_id,notification_type,category,severity,title,body,
        action_type,action_payload,operation_id,source_event_type,source_event_id,dedupe_key,data
      ) values (
        v_admin.id,'system_announcement','system','warning',
        ...
        'operation_details',jsonb_build_object('operation_id',v_operation.id,'public_token',v_operation.public_token),
        v_operation.id,'operation_pipeline_watchdog',v_operation.id::text,
  ...
  for v_operation in
    ...
    from private.operation_analysis_jobs j
    where j.updated_at >= v_now - interval '1 hour' and j.status in ('failed','dead_letter')
  loop
    for v_admin in select p.id from public.profiles p where p.global_role='platform_admin' and p.status='active'
    loop
      insert into public.notifications(
        ...
        v_admin.id,'system_announcement','system','error',
        case when v_operation.status='dead_letter' then 'عملية وصلت إلى Dead Letter' else 'فشل نهائي في تحليل عملية' end,
        left(coalesce(v_operation.last_error_message,'فشل محرك التحليل دون رسالة خطأ.'),1000),
        'operation_details',jsonb_build_object('operation_id',v_operation.operation_id),
        v_operation.operation_id,'operation_analysis_job_failure',v_operation.job_id::text,
        'pipeline-watchdog:job:'||v_operation.job_id::text||':'||v_operation.status,
        jsonb_build_object('job_id',v_operation.job_id,'job_status',v_operation.status,'error_code',v_operation.last_error_code)
      ) on conflict (recipient_user_id,dedupe_key) do nothing;
    end loop;
  end loop;
  ...
  if v_p95_breached then
    ...
    loop
      insert into public.notifications(
        ...
        v_admin.id,'system_announcement','system','warning',
        'تباطؤ في خط تحليل العمليات','تجاوز P95 لتحليل العمليات هدف 6 ثوانٍ خلال الساعة الأخيرة.',
        'none','{}'::jsonb,'operation_pipeline_slo',v_hour_bucket,
  ...
  v_result := jsonb_build_object(
    'ok',true,'checked_at',v_now,'stuck_operations',v_stuck,'failed_jobs',v_failed,
    'dead_letter_jobs',v_dead,'sample_operations',v_sample,
...
comment on function private.run_operation_pipeline_watchdog() is 'Automatic SANAD operation-pipeline watchdog: stuck >15s, failed/dead-letter analysis jobs, and rolling 1h P95 >6s; notifies platform admins through the canonical notifications table.';

do $$
declare v_job_id bigint;
begin
  select jobid into v_job_id from cron.job where jobname='sanad-operation-pipeline-watchdog-v1' limit 1;
  if v_job_id is not null then perform cron.unschedule(v_job_id); end if;
  perform cron.schedule('sanad-operation-pipeline-watchdog-v1','* * * * *','select private.run_operation_pipeline_watchdog();');
end;
$$;
