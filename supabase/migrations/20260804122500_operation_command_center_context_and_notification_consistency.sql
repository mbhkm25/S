...
-- Opening an operation is a read-only action. Operational state changes remain
-- confined to the existing claim/complete/review commands.
...
begin
  ...
  -- Reuse the access contract without consuming or mutating payment-inbox state.
  if not coalesce((public.open_operation_access_semantic_core(p_public_token,'app')->>'allowed')::boolean,false) then
  ...
  return jsonb_build_object(
    ...
    'received_at',v_operation.created_at,
...
-- The account was operationally verified before the business-account migration,
-- but both the account and its active primary identifier remained marked unverified.
update public.business_financial_accounts fa
set verification_status='verified', verified_at=coalesce(verified_at,now()), updated_at=now()
where fa.id in (
  select fi.financial_account_id
  from public.business_financial_identifiers fi
  where fi.identifier_value_normalized='254073867'
...
set verification_status='verified', verified_at=coalesce(verified_at,now()), updated_at=now()
where identifier_value_normalized='254073867'
  and status='active'
  and routing_enabled=true;
...
begin
  ...
  for v_user_id in
    ...
    where m.business_id=v_item.business_id and m.status='active'
      and private.has_business_payment_permission(v_item.business_id,'view',m.user_id)
  loop
    perform private.create_notification(
      v_user_id,'payment_inbox_new','business','info',
      concat('وردت دفعة جديدة إلى ',v_business.name),
      concat(coalesce(v_operation.amount::text,'—'),' ',coalesce(v_operation.currency,''),' عبر ',coalesce(v_operation.financial_entity,'جهة مالية أخرى'),case when v_account_suffix<>'' then concat(' · الحساب …',v_account_suffix) else '' end,'. راجع وارد المدفوعات.'),
      'business_operations',
      jsonb_build_object('business_id',v_item.business_id,'payment_inbox_id',v_item.id,'inbox_surface','payment-inbox','inbox_view','new','public_token',v_operation.public_token),
      null,v_item.business_id,v_item.operation_id,'business_payment_inbox',v_item.id::text,
      concat('payment_inbox_new:',v_item.id,':',v_user_id),
      jsonb_build_object('payment_inbox_id',v_item.id,'business_id',v_item.business_id,'source_mode',v_item.source_mode,'match_score',v_item.match_score,'public_token',v_operation.public_token),
...
begin
  ...
  for v_user_id in
    ...
    select m.user_id from public.business_team_members m
    where m.business_id=v_item.business_id and m.status='active'
      and private.has_business_payment_permission(v_item.business_id,'review',m.user_id)
  loop
    perform private.create_notification(
      v_user_id,'payment_inbox_review_required','business','warning',
      concat('دفعة محتملة تحتاج مراجعة في ',v_business.name),
      concat('طابق سند رقم الحساب',case when v_account_suffix<>'' then concat(' …',v_account_suffix) else '' end,' مع النشاط، لكن توثيق الحساب غير مكتمل. راجع العملية قبل اعتمادها.'),
      'business_operations',
      jsonb_build_object('business_id',v_item.business_id,'payment_inbox_id',v_item.id,'inbox_surface','payment-inbox-admin','inbox_view','review','public_token',v_operation.public_token),
      null,v_item.business_id,v_item.operation_id,'business_payment_inbox_review',v_item.id::text,
      concat('payment_inbox_review_required:',v_item.id,':',v_user_id),
      jsonb_build_object('payment_inbox_id',v_item.id,'business_id',v_item.business_id,'source_mode',v_item.source_mode,'match_score',v_item.match_score,'public_token',v_operation.public_token),
      now()+interval '30 days'
    );
  end loop;
