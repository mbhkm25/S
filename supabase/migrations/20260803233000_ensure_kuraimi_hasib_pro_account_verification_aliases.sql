...
begin
  if exists (
    select 1 from public.sanad_payment_accounts where account_number = '825121'
  ) then
    ...
    insert into public.sanad_payment_accounts (
      financial_entity, account_number, account_holder_name, currency,
      status, display_order, instructions, metadata
    ) values (
      'نقطة حاسب الكريمي', '825121', 'SANAD', 'YER',
      'active', 15,
