...
declare
  ...
  v_blocking_warnings jsonb := '[]'::jsonb;
begin
  select * into v_raw
  ...
  if v_raw.adapter_code not in ('edaa','edaa_v5')
    ...
    set normalization_status = 'quarantined', normalization_error = 'unsupported_edaa_sale_contract'
    where id = v_raw.id;
  ...
  if coalesce(v_bundle->>'schema_version','') <> 'ibex-1'
    ...
    update public.business_erp_raw_events
    set normalization_status = 'quarantined', normalization_error = 'unsupported_edaa_bundle_shape'
    where id = v_raw.id;
  ...
  if v_expected_line_count is not null and v_expected_line_count <> v_line_count then
    v_blocking_warnings := v_blocking_warnings || jsonb_build_array('sale_line_count_mismatch');
  end if;
  if v_expected_accounting_count is not null and v_expected_accounting_count <> v_accounting_line_count then
    v_blocking_warnings := v_blocking_warnings || jsonb_build_array('accounting_line_count_mismatch');
  end if;
  if v_expected_inventory_count is not null and v_expected_inventory_count <> v_inventory_line_count then
    v_blocking_warnings := v_blocking_warnings || jsonb_build_array('inventory_line_count_mismatch');
  end if;
  if not v_accounting_balanced then
    v_blocking_warnings := v_blocking_warnings || jsonb_build_array('accounting_not_balanced');
  end if;
  if not v_inventory_balanced then
    v_blocking_warnings := v_blocking_warnings || jsonb_build_array('inventory_not_balanced');
  end if;
  if v_currency_code is null then
    v_blocking_warnings := v_blocking_warnings || jsonb_build_array('currency_mapping_missing');
  end if;
  if v_customer_name is not null then
    v_warnings := v_warnings || jsonb_build_array('customer_identity_unresolved');
  end if;
  v_warnings := v_blocking_warnings || v_warnings;

  if v_deleted then
    v_status := 'cancelled';
  elsif jsonb_array_length(v_blocking_warnings) > 0 then
  ...
  v_activity_id := public.record_business_activity_from_erp_v1(
    ...
    jsonb_build_object(
      ...
      'blocking_warnings',v_blocking_warnings,
      'warnings',v_warnings
    )
  );

  return jsonb_build_object('status',case when v_status='pending_review' then 'normalized_with_warnings' else 'normalized' end,'raw_event_id',v_raw.id,'activity_event_id',v_activity_id,'activity_status',v_status,'amount',v_net,'currency',v_currency_code,'party_id',null,'blocking_warnings',v_blocking_warnings,'warnings',v_warnings);
