...
begin
  ...
  if v_latest.id is not null then
    select count(*)::integer,coalesce(sum(value::bigint),0)
    ...
    if v_latest.status in ('started','uploading') then
      select
        count(distinct e.integrity->>'table_name') filter (where e.integrity->>'table_name' is not null)::integer,
        coalesce(sum(case when coalesce(e.integrity->>'row_count','') ~ '^[0-9]+$' then (e.integrity->>'row_count')::bigint else 0 end),0),
        max(e.received_at)
