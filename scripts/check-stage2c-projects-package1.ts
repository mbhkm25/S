...
assert.match(projectData,/getUserBusinessContexts/);
...
assert.doesNotMatch(quickHome,/disabled=\{creating \|\| disableNew\}/, 'New chat must not be silently inert when the new RPC awaits rollout.');
assert.match(quickHome,/onNew=\{\(\) => void create\('personal'\)\}/);
assert.match(projectData,/openNewProjectConversation/);
assert.match(project,/وارد المدفوعات/);
