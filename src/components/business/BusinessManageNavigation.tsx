...
export default function BusinessManageNavigation({
  ...
  const renderButton = (
    tab: (typeof primaryTabs)[number] | (typeof secondaryTabs)[number],
    compact = false
  ) => {
    const Icon = tab.icon;
    const selected = activeTab === tab.id;
    const baseLabel = compact ? tab.shortLabel : tab.label;
    const label = tab.id === 'complaints' && complaintsCount > 0
      ? `${baseLabel} (${complaintsCount})`
      : baseLabel;
    return (
      <button
        key={tab.id}
      ...
      <div className="w-full lg:hidden">
        ...
        >
          <span className="flex min-w-0 items-center gap-3">
            <span className="rounded-xl bg-slate-100 p-2 text-slate-700"><ActiveIcon className="h-4 w-4" /></span>
