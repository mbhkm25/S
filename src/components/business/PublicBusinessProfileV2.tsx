...
export default function PublicBusinessProfileV2({slug,onNavigate,initialTab}:Props){
  ...
  useEffect(()=>{let active=true;(async()=>{setLoading(true);setError(null);try{
    ...
    const firstImages=catalog.map(item=>item.image_paths?.[0]).filter((path):path is string=>Boolean(path));
    const [entries,l,c]=await Promise.all([
  ...
  return <div className="min-h-screen bg-[#f7f8fa] pb-16 font-arabic" dir="rtl"><header className="sticky top-0 z-30 mx-1 flex items-center gap-3 rounded-b-2xl bg-white/95 p-3 shadow-[0_1px_16px_rgba(15,23,42,0.05)] backdrop-blur sm:mx-2"><button onClick={backToIntro} className="rounded-xl bg-slate-100 p-2.5"><ArrowRight className="h-4 w-4"/></button><div className="flex h-11 w-11 items-center justify-center overflow-hidden rounded-2xl bg-white shadow-sm">{logo?<img src={logo} decoding="async" className="h-full w-full object-cover"/>:<Store className="h-5 w-5"/>}</div><div className="min-w-0 flex-1"><h1 className="truncate text-sm font-bold">{profile.name}</h1><p className="text-[10px] text-slate-500">{profile.category_name||'عام'} · {profile.city}، {profile.governorate}</p></div>{whatsapp&&<a href={`https://wa.me/${whatsapp}`} className="rounded-xl bg-emerald-50 p-2.5 text-emerald-700"><MessageCircle className="h-5 w-5"/></a>}</header>
    <main className="mx-auto max-w-5xl space-y-4 px-2 py-3 sm:px-3">
      <section className="overflow-hidden rounded-[1.65rem] bg-gradient-to-br from-slate-950 via-slate-900 to-emerald-950 p-5 text-white shadow-[0_16px_42px_rgba(15,23,42,0.16)]">
        <div className="flex items-center gap-2"><span className="text-[9px] font-bold text-emerald-300">الهوية التجارية</span>{profile.verification_status==='verified'&&<ShieldCheck className="h-4 w-4 text-emerald-300"/>}</div>
        <h2 className="mt-2 text-lg font-bold">{profile.name}</h2><p className="mt-1 text-xs text-white/65">{profile.display_tagline||profile.category_name||'نشاط تجاري'}</p>
