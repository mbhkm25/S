...
const SECTIONS: Array<{id:Section;label:string}> = [
  {id:'catalog',label:'الكتالوج'}, {id:'overview',label:'نظرة عامة'},
  {id:'hours',label:'ساعات العمل'}, {id:'accounts',label:'الحسابات المالية'},
  {id:'contact',label:'التواصل والموقع'}
];
...
function setProfileView(view: 'intro' | 'details', section?: Section) {
  ...
  if (view === 'intro') {
    ...
    url.searchParams.set('section', section || 'catalog');
  }
...
export default function PublicBusinessProfileV2({slug,onNavigate,initialTab}:Props){
  ...
  const [section,setSection]=useState<Section>(requestedSection && SECTIONS.some(x=>x.id===requestedSection) ? requestedSection : initialTab==='financial'?'accounts':initialTab==='products'||initialTab==='services'?'catalog':'overview');
  ...
  const socialLinks = [
    links.website && { label:'الموقع الإلكتروني', href:links.website, Icon:Globe2 },
    links.facebook && { label:'فيسبوك', href:links.facebook, Icon:Facebook },
    links.instagram && { label:'إنستغرام', href:links.instagram, Icon:Instagram },
    (links.twitter || links.x) && { label:'X', href:links.twitter || links.x || '', Icon:ExternalLink }
  ...
  if(mode==='intro')return <div className="min-h-screen bg-white p-3 font-arabic" dir="rtl"><div className="relative mx-auto min-h-[calc(100dvh-24px)] max-w-xl overflow-hidden rounded-[2rem] bg-slate-200 shadow-[0_24px_70px_rgba(15,23,42,0.16)]">
    {cover?<img src={cover} alt={profile.name} className="absolute inset-0 h-full w-full object-cover"/>:<div className="absolute inset-0 bg-gradient-to-br from-slate-300 to-slate-600"/>}<div className="absolute inset-0 bg-gradient-to-b from-transparent via-slate-950/5 to-slate-950/90"/>
    ...
    <div className="absolute inset-x-0 bottom-0 z-10 space-y-4 p-6 text-white">
      <div className="flex items-end gap-4"><div className="flex h-20 w-20 items-center justify-center overflow-hidden rounded-3xl bg-white shadow-xl">{logo?<img src={logo} className="h-full w-full object-cover"/>:<Store className="h-8 w-8 text-slate-500"/>}</div><div className="min-w-0 flex-1"><div className="flex items-center gap-2"><h1 className="truncate text-2xl font-bold">{profile.name}</h1>{profile.verification_status==='verified'&&<ShieldCheck className="h-5 w-5 text-emerald-300"/>}</div><p className="mt-1 text-xs text-white/75">{profile.display_tagline||profile.category_name||'نشاط تجاري'}</p><div className="mt-2 flex flex-wrap gap-x-3 gap-y-1 text-[11px] text-white/70"><span className="flex items-center gap-1"><Package className="h-3.5 w-3.5"/>{profile.category_name||'عام'}</span><span className="flex items-center gap-1"><MapPin className="h-3.5 w-3.5"/>{profile.city}، {profile.governorate}</span>{catalog.length>0&&<span>{catalog.length} عناصر في الكتالوج</span>}</div></div></div>
      ...
      {socialLinks.length>0&&<div className="flex flex-wrap gap-2">{socialLinks.map(({label,href,Icon})=><a key={label} href={href} target="_blank" rel="noreferrer" className="flex items-center gap-1.5 rounded-full bg-white/12 px-3 py-2 text-[10px] font-bold backdrop-blur"><Icon className="h-3.5 w-3.5"/>{label}</a>)}</div>}
      <div className="grid grid-cols-2 gap-3"><button onClick={()=>void join()} disabled={linked||linking} className="flex items-center justify-center gap-2 rounded-2xl bg-white p-3.5 text-xs font-bold text-slate-950 shadow-lg disabled:bg-emerald-100 disabled:text-emerald-800">{linking?<Loader2 className="h-4 w-4 animate-spin"/>:<UserCheck className="h-4 w-4"/>}{linked?'مرتبط بالنشاط':'الارتباط كعميل'}</button><button onClick={()=>openDetails(catalog.length?'catalog':'overview')} className="flex items-center justify-center gap-2 rounded-2xl bg-white/14 p-3.5 text-xs font-bold backdrop-blur">استعراض الملف<ExternalLink className="h-4 w-4"/></button></div>
    </div>
  </div></div>;

  return <div className="min-h-screen bg-[#f7f8fa] pb-16 font-arabic" dir="rtl"><header className="sticky top-0 z-30 flex items-center gap-3 bg-white/95 p-3 shadow-[0_1px_16px_rgba(15,23,42,0.05)] backdrop-blur"><button onClick={backToIntro} className="rounded-xl bg-slate-100 p-2.5"><ArrowRight className="h-4 w-4"/></button><div className="flex h-11 w-11 items-center justify-center overflow-hidden rounded-2xl bg-white shadow-sm">{logo?<img src={logo} className="h-full w-full object-cover"/>:<Store className="h-5 w-5"/>}</div><div className="min-w-0 flex-1"><h1 className="truncate text-sm font-bold">{profile.name}</h1><p className="text-[10px] text-slate-500">{profile.category_name||'عام'} · {profile.city}، {profile.governorate}</p></div>{whatsapp&&<a href={`https://wa.me/${whatsapp}`} className="rounded-xl bg-emerald-50 p-2.5 text-emerald-700"><MessageCircle className="h-5 w-5"/></a>}</header>
    <main className="mx-auto max-w-5xl space-y-5 p-3"><section className="rounded-[1.75rem] bg-white shadow-[0_10px_30px_rgba(15,23,42,0.05)]"><button onClick={()=>setMenu(v=>!v)} className="flex w-full items-center gap-3 p-4"><Store className="h-5 w-5"/><div className="flex-1 text-right"><span className="block text-[9px] text-slate-400">قسم الملف العام</span><strong className="text-sm">{SECTIONS.find(x=>x.id===section)?.label}</strong></div><ChevronDown className="h-5 w-5"/></button>{menu&&<div className="grid grid-cols-2 gap-2 px-3 pb-3">{SECTIONS.map(x=><button key={x.id} onClick={()=>selectSection(x.id)} className={`rounded-xl p-3 text-xs font-bold ${section===x.id?'bg-slate-900 text-white':'bg-slate-50 text-slate-700'}`}>{x.label}</button>)}</div>}</section>
    {section==='catalog'&&<section className="space-y-3"><div><h2 className="text-base font-bold">كتالوج النشاط</h2><p className="mt-1 text-[10px] text-slate-500">العناصر والخدمات والعروض المنشورة</p></div>{catalog.length?<div className="grid grid-cols-2 gap-3 sm:grid-cols-3">{catalog.map(item=><article key={item.id} className="overflow-hidden rounded-[1.5rem] bg-white shadow-[0_10px_28px_rgba(15,23,42,0.06)]"><button onClick={()=>openItem(item.id)} className="w-full text-right"><div className="aspect-[4/3] bg-slate-100">{images[item.id]?<img src={images[item.id]} className="h-full w-full object-cover"/>:<div className="flex h-full items-center justify-center"><ImageIcon className="h-8 w-8 text-slate-300"/></div>}</div><div className="space-y-2 p-3"><span className="text-[9px] text-slate-400">{item.item_type||'عنصر'}</span><h3 className="line-clamp-1 text-xs font-bold">{item.title}</h3>{item.description&&<p className="line-clamp-2 text-[10px] leading-5 text-slate-500">{item.description}</p>}<p className="text-[11px] font-bold text-slate-900">{priceLabel(item)}</p></div></button></article>)}</div>:<p className="rounded-[1.5rem] bg-white p-8 text-center text-xs text-slate-400 shadow-sm">لا توجد عناصر منشورة.</p>}</section>}
    {section==='overview'&&<section className="rounded-[1.75rem] bg-white p-5 shadow-[0_10px_30px_rgba(15,23,42,0.05)]"><div className="flex items-center gap-2"><h2 className="text-lg font-bold">{profile.name}</h2>{profile.verification_status==='verified'&&<ShieldCheck className="h-5 w-5 text-emerald-600"/>}</div><p className="mt-3 text-xs leading-7 text-slate-600">{profile.description||'لا يوجد وصف.'}</p><div className="mt-5 grid grid-cols-2 gap-3"><div className="rounded-2xl bg-slate-50 p-3"><span className="text-[10px] text-slate-400">التصنيف</span><strong className="mt-1 block text-xs">{profile.category_name||'عام'}</strong></div><div className="rounded-2xl bg-slate-50 p-3"><span className="text-[10px] text-slate-400">الموقع</span><strong className="mt-1 block text-xs">{profile.city}، {profile.governorate}</strong></div></div></section>}
    ...
    {section==='contact'&&<section className="space-y-4 rounded-[1.75rem] bg-white p-5 shadow-[0_10px_30px_rgba(15,23,42,0.05)]"><h2 className="text-sm font-bold">التواصل والموقع</h2><p className="flex items-start gap-2 text-xs text-slate-600"><MapPin className="h-4 w-4"/>{profile.address_text||`${profile.city}، ${profile.governorate}`}</p>{socialLinks.length>0&&<div className="grid grid-cols-2 gap-2">{socialLinks.map(({label,href,Icon})=><a key={label} href={href} target="_blank" rel="noreferrer" className="flex items-center gap-2 rounded-xl bg-slate-50 p-3 text-[10px] font-bold"><Icon className="h-4 w-4"/>{label}</a>)}</div>}{whatsapp&&<a href={`https://wa.me/${whatsapp}`} className="flex justify-center gap-2 rounded-2xl bg-emerald-600 p-3 text-xs font-bold text-white"><MessageCircle className="h-4 w-4"/>تواصل عبر واتساب</a>}</section>}
    </main></div>;
}
