...
function logoPaths(entity: string): string[] {
  return LOGOS.find(entry => entry.names.some(name => entity.includes(name)))?.paths || [];
}
...
function EntityLogo({ entity }: { entity: string }) {
  const candidates = logoPaths(entity);
...
export default function PaymentInbox({ admin = false, onNavigate }: PaymentInboxProps) {
  ...
  const loadContexts = useCallback(async () => {
    ...
    const selected = next.find(item => item.business_id === requested) || next[0];
  ...
  const loadItems = useCallback(async () => {
    ...
    setError(null);
    try {
      const next = await getPaymentInbox(businessId, view);
      setItems(next);
    } catch (cause) {
      setItems([]);
  ...
  const cashierTabs: Array<{ value: PaymentInboxView; label: string }> = [
    { value: 'new', label: 'جديدة' },
    { value: 'mine', label: 'لدي' }
  ];
  const adminTabs: Array<{ value: PaymentInboxView; label: string }> = [
    { value: 'team_active', label: 'لدى الفريق' },
    { value: 'review', label: 'تحتاج مراجعة' },
    { value: 'completed', label: 'مكتملة' },
    { value: 'all', label: 'كل العمليات' }
  ];
  const tabs = admin ? adminTabs : cashierTabs;
      ...
      <div className="rounded-3xl bg-slate-950 p-4 text-white shadow-sm">
        <div className="flex items-center justify-between gap-3">
          <div className="min-w-0">
            <h1 className="text-lg font-black">{admin ? 'إدارة وارد المدفوعات' : 'وارد المدفوعات'}</h1>
            <p className="mt-1 text-[10px] text-slate-400">
              {admin ? 'متابعة عمليات الفريق واتخاذ القرارات الإشرافية' : 'استلام أحدث العمليات والتحقق منها بسرعة'}
            </p>
          </div>
          <button
            type="button"
            onClick={() => void loadItems()}
            className="inline-flex h-10 w-10 shrink-0 items-center justify-center rounded-2xl border border-white/10 bg-white/10"
            aria-label="تحديث"
          >
        ...
        <div className="mt-4 grid gap-3 sm:grid-cols-[minmax(0,1fr)_auto] sm:items-end">
          <select
            ...
            className="h-11 w-full rounded-2xl border border-white/10 bg-white/10 px-3 text-xs font-bold text-white outline-none"
            aria-label="اختيار النشاط التجاري"
          >
            {contexts.map(item => <option key={item.business_id} value={item.business_id} className="text-slate-900">{item.business_name}</option>)}
          </select>
          {!admin && context?.is_supervisor && (
            <button
              type="button"
              onClick={() => {
                const params = new URLSearchParams({ view: 'payment-inbox-admin', business_id: businessId });
                window.history.pushState({}, '', `/business/manage/operations?${params.toString()}`);
                window.dispatchEvent(new PopStateEvent('popstate'));
              }}
              className="inline-flex h-11 items-center justify-center gap-2 rounded-2xl border border-white/10 bg-white/10 px-4 text-xs font-black"
            >
        ...
        <div className="mt-3 grid grid-cols-2 gap-2 sm:flex">
          {tabs.map(tab => (
            <button
              ...
              className={`h-10 rounded-2xl px-4 text-xs font-black transition ${view === tab.value ? 'bg-white text-slate-950' : 'border border-white/10 bg-white/5 text-slate-300'}`}
            >
              {tab.label}
            </button>
          ))}
              ...
              <article key={item.id} className="overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm">
                <div className="grid gap-4 p-4 sm:grid-cols-[minmax(0,1fr)_auto] sm:items-center">
                  <div className="flex min-w-0 items-center gap-3">
                    <div className="flex h-12 w-20 shrink-0 items-center justify-start">
                  ...
                  <div className="flex items-baseline justify-end gap-2 sm:min-w-[150px]">
                    ...
                    <span className={`rounded-full border px-2 py-1 text-[10px] font-black ${currencyTone(item.currency)}`}>{item.currency || '—'}</span>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-2 gap-2 border-t border-slate-100 bg-slate-50/70 p-3 sm:grid-cols-4">
                  ...
                  {view === 'new' && (
                    <>
                      <button type="button" disabled={isBusy} onClick={() => void runAction(item, 'claim')} className="inline-flex min-h-11 items-center justify-center gap-2 rounded-2xl bg-emerald-600 px-3 text-[11px] font-black text-white disabled:opacity-60">
                        {isBusy ? <Loader2 className="h-4 w-4 animate-spin" /> : <Inbox className="h-4 w-4" />} استلام العملية
                      </button>
                      <button type="button" disabled={isBusy} onClick={() => void runAction(item, 'claim-verify')} className="inline-flex min-h-11 items-center justify-center gap-2 rounded-2xl bg-blue-600 px-3 text-[11px] font-black text-white disabled:opacity-60">
                  ...
                  {view === 'mine' && (
                    <button type="button" disabled={isBusy} onClick={() => void runAction(item, 'complete')} className="inline-flex min-h-11 items-center justify-center gap-2 rounded-2xl bg-emerald-600 px-3 text-[11px] font-black text-white disabled:opacity-60">
