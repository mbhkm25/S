    ...
    <section className="space-y-4 font-arabic" dir="rtl">
      <header className="rounded-3xl border border-slate-200 bg-white p-4 shadow-sm">
        <div className="flex items-start justify-between gap-3">
          <div className="min-w-0"><h1 className="text-lg font-black text-slate-950">{admin ? 'إدارة وارد المدفوعات' : 'وارد المدفوعات'}</h1><p className="mt-1 text-[10px] leading-5 text-slate-500">{admin ? 'متابعة عمليات الفريق والحالات التي تحتاج قرارًا إشرافيًا.' : 'استلام أحدث العمليات المرتبطة بالنشاط والتحقق منها.'}</p></div>
          <button type="button" onClick={() => void loadItems()} className="inline-flex h-10 w-10 shrink-0 items-center justify-center rounded-2xl border border-slate-200 bg-slate-50 text-slate-700" aria-label="تحديث وارد المدفوعات"><RefreshCw className="h-4 w-4" /></button>
        </div>
        ...
        <div className="space-y-3">{items.map(item => {
          ...
          return <article key={item.id} className="overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm">
            <div className="grid gap-4 p-4 sm:grid-cols-[minmax(0,1fr)_auto] sm:items-center">
              <div className="flex min-w-0 items-center gap-3"><div className="flex h-12 w-20 shrink-0 items-center justify-start"><EntityLogo entity={entity} /></div><div className="min-w-0"><h3 className="truncate text-sm font-black text-slate-900">{entity}</h3><p className="mt-1 truncate text-xs font-medium text-slate-500">{identity}</p><div className="mt-2 flex flex-wrap items-center gap-x-4 gap-y-1.5 text-[12px] leading-5 text-slate-500"><span className="inline-flex items-center gap-1.5"><Clock3 className="h-3.5 w-3.5" />{formatTime(item.created_at || item.transaction_datetime)}</span>{point && <span>الحساب/النقطة: <b dir="ltr" className="inline-block font-mono text-slate-700">{toLatinDigits(point)}</b></span>}{item.reference_number && <span>المرجع: <b dir="ltr" className="inline-block font-mono text-slate-700">{toLatinDigits(item.reference_number)}</b></span>}</div></div></div>
              <div className="flex items-baseline justify-end gap-2 sm:min-w-[150px]"><strong className="text-4xl font-black tracking-tight text-slate-950">{formatAmount(item.amount)}</strong><span className={`rounded-full border px-2 py-1 text-[10px] font-black ${currencyClass(item.currency)}`}>{item.currency || '—'}</span></div>
            </div>
            {admin && item.claimed_by_name && <div className="mx-4 mb-3 flex items-center gap-2 rounded-2xl bg-slate-50 px-3 py-2 text-[10px] text-slate-600"><UserRoundCheck className="h-4 w-4 text-slate-400" /><span>المسؤول الحالي: <b>{item.claimed_by_name}</b></span></div>}
            <div className="grid grid-cols-2 gap-2 border-t border-slate-100 bg-slate-50/70 p-3 sm:grid-cols-4">
              <button type="button" disabled={accessChecking} onClick={() => void openProtected(item)} className="inline-flex min-h-11 items-center justify-center gap-2 rounded-2xl border border-slate-200 bg-white px-3 text-[11px] font-black text-slate-700 disabled:opacity-60"><Eye className="h-4 w-4" /> فتح السجل</button>
              {canClaim && <><button type="button" disabled={accessChecking} onClick={() => void openProtected(item, 'claim')} className="inline-flex min-h-11 items-center justify-center gap-2 rounded-2xl bg-emerald-600 px-3 text-[11px] font-black text-white disabled:opacity-60"><Inbox className="h-4 w-4" /> استلام العملية</button><button type="button" disabled={accessChecking} onClick={() => void openProtected(item, 'claim_verify')} className="inline-flex min-h-11 items-center justify-center gap-2 rounded-2xl bg-blue-600 px-3 text-[11px] font-black text-white disabled:opacity-60"><CheckCircle2 className="h-4 w-4" /> استلام وتحقق</button></>}
              {canComplete && <button type="button" disabled={accessChecking} onClick={() => void openProtected(item, 'complete')} className="inline-flex min-h-11 items-center justify-center gap-2 rounded-2xl bg-emerald-600 px-3 text-[11px] font-black text-white disabled:opacity-60"><CheckCircle2 className="h-4 w-4" /> إكمال العملية</button>}
              <button type="button" disabled={accessChecking} onClick={() => void openProtected(item, 'open_original')} className="inline-flex min-h-11 items-center justify-center gap-2 rounded-2xl border border-violet-200 bg-violet-50 px-3 text-[11px] font-black text-violet-700 disabled:opacity-60"><FileText className="h-4 w-4" /> فتح الملف الأصلي</button>
            </div>
          </article>;
        })}</div>
      )}
