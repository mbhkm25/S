...
import {
  ...
  MessageCircle, Minus, PackageCheck, Plus, RotateCcw, Send, ShoppingBag, Trash2, Truck, X
} from 'lucide-react';
import {
...
export default function PublicCatalogOrderExperience({ business, items, images, itemLabel, onOpenItem }: Props) {
  ...
  const totals = useMemo<Record<string, number>>(() => cartTotals(cart), [cart]);
  const totalEntries = useMemo(() => Object.entries(totals) as Array<[string, number]>, [totals]);
  ...
  return <section className="space-y-3">
    ...
    <div className={cardGrid}>{items.map(item => {
      ...
      return <article key={item.id} className={`overflow-hidden rounded-[1.5rem] bg-white shadow-sm ${effectClass(settings)} ${settings.card_style==='compact'?'flex min-h-28':''} ${unavailable?'opacity-75':''}`}>
    ...
    {settings.ordering_enabled&&count>0&&<button type="button" onClick={()=>setSheetOpen(true)} className="fixed bottom-5 left-1/2 z-40 flex w-[calc(100%-24px)] max-w-lg -translate-x-1/2 items-center gap-3 rounded-2xl bg-slate-950 p-3.5 text-white shadow-[0_18px_45px_rgba(15,23,42,.3)]"><span className="flex h-9 w-9 items-center justify-center rounded-xl bg-emerald-400 text-sm font-bold text-emerald-950">{count}</span><span className="min-w-0 flex-1 text-right"><strong className="block text-xs">قائمة الطلب</strong><span className="text-[9px] text-white/60">مرجع {orderReference}</span></span><ShoppingBag className="h-5 w-5"/></button>}

    {sheetOpen&&settings.ordering_enabled&&<div className="fixed inset-0 z-50 flex items-end justify-center bg-slate-950/45 p-0 sm:items-center sm:p-4" role="dialog" aria-modal="true"><div className="max-h-[92dvh] w-full max-w-xl overflow-y-auto rounded-t-[2rem] bg-white p-4 shadow-2xl sm:rounded-[2rem]">
      <div className="flex items-center gap-3"><span className="flex h-10 w-10 items-center justify-center rounded-2xl bg-slate-100"><ShoppingBag className="h-5 w-5"/></span><div className="flex-1"><h2 className="text-sm font-bold">مراجعة الطلب</h2><p className="text-[9px] text-slate-500">مرجع الطلب: {orderReference}</p></div><button type="button" onClick={()=>setSheetOpen(false)} className="rounded-xl bg-slate-100 p-2"><X className="h-4 w-4"/></button></div>
      <div className="mt-4 space-y-2">{cart.map(item=><article key={item.id} className="rounded-2xl border border-slate-200 p-3"><div className="flex items-start gap-2"><div className="min-w-0 flex-1"><h3 className="truncate text-xs font-bold">{item.title}</h3>{settings.show_prices&&<p className="mt-1 text-[10px] text-slate-500">{formatCatalogPrice(item.price,item.currency,settings.price_display,settings.missing_price_label)}</p>}</div><button type="button" onClick={()=>remove(item.id)} className="rounded-lg p-1.5 text-rose-600"><Trash2 className="h-4 w-4"/></button></div><div className="mt-3 flex items-center gap-2"><button type="button" onClick={()=>updateQuantity(item.id,-1)} className="rounded-xl border p-2"><Minus className="h-3.5 w-3.5"/></button><strong className="min-w-8 text-center text-xs">{item.quantity}</strong><button type="button" onClick={()=>updateQuantity(item.id,1)} className="rounded-xl border p-2"><Plus className="h-3.5 w-3.5"/></button></div>{settings.allow_item_notes&&<input value={item.note||''} onChange={event=>updateNote(item.id,event.target.value)} maxLength={180} placeholder="ملاحظة على هذا العنصر" className="mt-3 w-full rounded-xl border border-slate-200 bg-slate-50 p-2.5 text-[10px]"/>}</article>)}</div>
      {settings.show_total&&totalEntries.length>0&&<div className="mt-4 rounded-2xl bg-slate-50 p-3"><p className="text-[10px] font-bold text-slate-600">الإجمالي بحسب العملة</p><div className="mt-2 flex flex-wrap gap-2">{totalEntries.map(([currency,total])=><strong key={currency} className="rounded-full bg-white px-3 py-2 text-[10px] shadow-sm">{formatCatalogPrice(total,currency,settings.price_display,settings.missing_price_label)}</strong>)}</div></div>}
      <div className="mt-4 grid gap-2 sm:grid-cols-2"><input value={customer.name||''} onChange={event=>setCustomer(current=>({...current,name:event.target.value}))} placeholder={`اسم العميل${settings.require_customer_name?' *':''}`} className="rounded-xl border border-slate-200 bg-slate-50 p-3 text-xs"/><input value={customer.phone||''} onChange={event=>setCustomer(current=>({...current,phone:event.target.value}))} placeholder="رقم التواصل" className="rounded-xl border border-slate-200 bg-slate-50 p-3 text-xs"/><input value={customer.area||''} onChange={event=>setCustomer(current=>({...current,area:event.target.value}))} placeholder="المنطقة" className="rounded-xl border border-slate-200 bg-slate-50 p-3 text-xs"/><input value={customer.address||''} onChange={event=>setCustomer(current=>({...current,address:event.target.value}))} placeholder={`العنوان${settings.require_address||deliverySettings.require_customer_address?' *':''}`} className="rounded-xl border border-slate-200 bg-slate-50 p-3 text-xs"/><select value={customer.paymentMethod||'unspecified'} onChange={event=>setCustomer(current=>({...current,paymentMethod:event.target.value as CatalogCustomerDetails['paymentMethod']}))} className="rounded-xl border border-slate-200 bg-slate-50 p-3 text-xs sm:col-span-2"><option value="unspecified">حالة الدفع غير محددة</option><option value="paid">مدفوع</option><option value="cash_on_delivery">الدفع عند الاستلام</option></select><textarea value={customer.note||''} onChange={event=>setCustomer(current=>({...current,note:event.target.value}))} rows={2} maxLength={240} placeholder="ملاحظات عامة" className="rounded-xl border border-slate-200 bg-slate-50 p-3 text-xs sm:col-span-2"/></div>
      {sentState&&<div className="mt-3 flex items-center gap-2 rounded-2xl bg-emerald-50 p-3 text-[10px] text-emerald-700"><CheckCircle2 className="h-4 w-4"/>{sentState==='merchant'?'تم فتح محادثة المتجر بهذا المرجع.':'تم فتح محادثة شركة التوصيل بالمرجع نفسه.'}</div>}
