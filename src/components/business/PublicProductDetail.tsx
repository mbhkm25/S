...
function typeLabel(value?: string) {
  return ({ product:'منتج', service:'خدمة', digital:'عنصر رقمي', offer:'عرض', subscription:'اشتراك', other:'عنصر' } as Record<string,string>)[value || ''] || 'عنصر كتالوج';
}
...
export default function PublicProductDetail({ businessSlug, productId, onNavigate }: PublicProductDetailProps) {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      try {
        ...
        if (!found) { setError('لم يتم العثور على العنصر المطلوب في كتالوج النشاط.'); return; }
        setProfile(business); setItem(found);
  ...
  const handleShare = async () => {
    ...
    try {
      if (navigator.share) {
        await navigator.share({ title: item.title, text: `شاهد ${item.title} من كتالوج ${profile?.name || 'النشاط'} على سند`, url: itemUrl });
        return;
      }
    } catch (caught) {
  ...
  const goBack = () => {
    ...
    else onNavigate('public-business-profile', businessSlug);
  };
  ...
  return (
    <div className="min-h-screen bg-[#f7f8fa] pb-24 font-arabic text-right" dir="rtl">
      <header className="sticky top-0 z-40 flex items-center justify-between bg-white/95 px-3 py-3 shadow-[0_1px_16px_rgba(15,23,42,0.05)] backdrop-blur">
        <button onClick={goBack} className="rounded-xl bg-slate-100 p-2.5 text-slate-700" aria-label="العودة"><ArrowRight className="h-4 w-4" /></button>
        <h1 className="max-w-[58%] truncate text-xs font-bold text-slate-950">{item.title}</h1>
        <button onClick={() => void handleShare()} className="rounded-xl bg-slate-100 p-2.5 text-slate-700" aria-label="مشاركة">
          {shareStatus === 'copied' ? <Check className="h-4 w-4 text-emerald-600" /> : shareStatus === 'failed' ? <Copy className="h-4 w-4 text-rose-600" /> : <Share2 className="h-4 w-4" />}
        </button>
      </header>
      ...
      <main className="mx-auto max-w-2xl space-y-4 px-3 py-4">
        <section className="overflow-hidden rounded-[2rem] bg-white shadow-[0_16px_44px_rgba(15,23,42,0.07)]">
          ...
          <div className="space-y-5 p-5">
            <div className="flex items-start justify-between gap-4">
              <div className="min-w-0 flex-1"><p className="text-[10px] text-slate-400">{typeLabel(item.item_type)}</p><h2 className="mt-1 text-xl font-bold text-slate-950">{item.title}</h2></div>
              <span className={`rounded-full px-3 py-1.5 text-[9px] font-bold ${item.availability_status === 'unavailable' ? 'bg-rose-50 text-rose-700' : 'bg-emerald-50 text-emerald-700'}`}>{availabilityLabel(item.availability_status)}</span>
            </div>
            <div className="flex items-center justify-between rounded-2xl bg-slate-50 px-4 py-3"><span className="text-[10px] font-bold text-slate-400">السعر</span><strong className="text-sm font-bold text-slate-950">{priceLabel(item)}</strong></div>
            {item.description ? <p className="whitespace-pre-line text-xs leading-7 text-slate-600">{item.description}</p> : <p className="text-xs text-slate-400">لم يضف النشاط وصفًا تفصيليًا لهذا العنصر بعد.</p>}
            <div className="grid grid-cols-2 gap-2 text-[10px]"><div className="rounded-2xl bg-slate-50 p-3"><span className="text-slate-400">النوع</span><strong className="mt-1 block">{typeLabel(item.item_type)}</strong></div><div className="rounded-2xl bg-slate-50 p-3"><span className="text-slate-400">التوفر</span><strong className="mt-1 block">{availabilityLabel(item.availability_status)}</strong></div></div>
          </div>
        </section>

        <section className="rounded-[1.5rem] bg-white p-4 shadow-[0_10px_28px_rgba(15,23,42,0.05)]">
      ...
      {whatsappUrl && <div className="fixed inset-x-0 bottom-0 z-40 bg-white/95 px-3 pt-3 pb-[calc(0.75rem+env(safe-area-inset-bottom))] shadow-[0_-8px_30px_rgba(15,23,42,0.06)] backdrop-blur"><a href={whatsappUrl} target="_blank" rel="noreferrer" className="mx-auto flex max-w-2xl items-center justify-center gap-2 rounded-2xl bg-slate-900 py-3.5 text-xs font-bold text-white"><MessageCircle className="h-5 w-5" /> استفسار عبر واتساب</a></div>}
    </div>
  );
}
