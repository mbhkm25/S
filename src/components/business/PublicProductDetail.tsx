...
function priceLabel(item:CatalogItem){if(item.price!=null)return`${new Intl.NumberFormat('en-US',{maximumFractionDigits:2}).format(item.price)}${item.currency?` ${item.currency}`:''}`;return legacyPrice(item)||'السعر عند الطلب';}
...
export default function PublicProductDetail({businessSlug,productId,onNavigate}:PublicProductDetailProps){
  ...
  useEffect(()=>{let active=true;(async()=>{setLoading(true);setError(null);setActiveImage(0);try{
    const business=await getPublicBusinessProfile(decodeURIComponent(businessSlug)) as any;
    if(!active)return;
    const items=Array.isArray(business.catalog_items)?business.catalog_items as CatalogItem[]:[];
    const decodedProductId=decodeURIComponent(productId);
    const found=items.find(candidate=>candidate.id===decodedProductId)||null;
    if(!found){setError('لم يتم العثور على العنصر المطلوب في كتالوج النشاط.');return;}
    setProfile(business);setItem(found);
    const paths=Array.isArray(found.image_paths)?found.image_paths.filter(Boolean):[];
    const urls=(await Promise.all(paths.map(path=>getBusinessMediaSignedUrl(path).catch(()=>'')))).filter(Boolean);
  ...
  return <div className="min-h-screen bg-[#f7f8fa] pb-24 font-arabic text-right" dir="rtl">
    ...
    <main className="w-full space-y-3 px-0.5 py-3 sm:px-3 lg:px-5">
      <section className="overflow-hidden border-y border-slate-200 bg-white sm:rounded-[2rem] sm:border sm:shadow-[0_16px_44px_rgba(15,23,42,0.07)]">
        <div className="aspect-[4/3] bg-slate-100">{currentImage?<img src={currentImage} alt={item.title} className="h-full w-full object-cover"/>:<div className="flex h-full items-center justify-center"><ImageIcon className="h-12 w-12 text-slate-300"/></div>}</div>
        {imageUrls.length>1&&<div className="flex gap-2 overflow-x-auto px-3 py-3 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">{imageUrls.map((url,index)=><button key={url} type="button" onClick={()=>setActiveImage(index)} className={`h-16 w-20 shrink-0 overflow-hidden rounded-xl border-2 ${activeImage===index?'border-slate-900':'border-transparent'}`}><img src={url} alt={`${item.title} ${index+1}`} className="h-full w-full object-cover"/></button>)}</div>}
        <div className="space-y-5 p-4 sm:p-5"><div className="flex items-start justify-between gap-4"><div className="min-w-0 flex-1"><p className="text-[10px] text-slate-400">{typeLabel(item.item_type)}</p><h2 className="mt-1 text-xl font-bold text-slate-950">{item.title}</h2></div><span className={`rounded-full px-3 py-1.5 text-[9px] font-bold ${item.availability_status==='unavailable'?'bg-rose-50 text-rose-700':'bg-emerald-50 text-emerald-700'}`}>{availabilityLabel(item.availability_status)}</span></div><div className="flex items-center justify-between rounded-2xl bg-slate-50 px-4 py-3"><span className="text-[10px] font-bold text-slate-400">السعر</span><strong className="text-sm font-bold text-slate-950">{priceLabel(item)}</strong></div>{item.description?<p className="whitespace-pre-line text-xs leading-7 text-slate-600">{item.description}</p>:<p className="text-xs text-slate-400">لم يضف النشاط وصفًا تفصيليًا لهذا العنصر بعد.</p>}<div className="grid grid-cols-2 gap-2 text-[10px]"><div className="rounded-2xl bg-slate-50 p-3"><span className="text-slate-400">النوع</span><strong className="mt-1 block">{typeLabel(item.item_type)}</strong></div><div className="rounded-2xl bg-slate-50 p-3"><span className="text-slate-400">التوفر</span><strong className="mt-1 block">{availabilityLabel(item.availability_status)}</strong></div></div></div>
      </section>
      <section className="border-y border-slate-200 bg-white p-4 sm:rounded-[1.5rem] sm:border sm:shadow-[0_10px_28px_rgba(15,23,42,0.05)]"><button onClick={()=>onNavigate('public-business-profile',businessSlug)} className="flex w-full items-center gap-3 text-right"><Package className="h-5 w-5 text-slate-600"/><div><p className="text-xs font-bold text-slate-900">من كتالوج {profile?.name}</p><p className="mt-1 text-[10px] text-slate-400">{profile?.category_name||'نشاط تجاري'} · {profile?.city}، {profile?.governorate}</p></div>{profile?.verification_status==='verified'&&<ShieldCheck className="mr-auto h-5 w-5 text-emerald-600"/>}</button></section>
    </main>
