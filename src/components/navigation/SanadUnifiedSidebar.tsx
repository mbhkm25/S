...
export default function SanadUnifiedSidebar({
  ...
  return (
    <>
      ...
      >
        <div data-sanad-brand-lockup="vertical" className={`shrink-0 border-b border-[var(--sanad-border-subtle)] px-3 pb-2 pt-3 ${collapsed ? 'lg:px-1.5' : ''}`}>
          <div className={`flex items-start justify-between gap-1 ${collapsed ? 'lg:flex-col lg:items-center' : ''}`}>
            <a
              ...
              aria-label={collapsed ? 'توسيع الشريط الجانبي' : 'العودة إلى اليوم'}
              className="sanad-focus-ring flex min-w-0 items-start rounded-lg"
            >
              <img src={`${import.meta.env.BASE_URL}logo.png`} alt="سند"
                className={`w-auto shrink-0 object-contain object-right ${collapsed ? 'h-11 max-w-[132px] lg:h-8 lg:max-w-[44px]' : 'h-11 max-w-[132px]'}`}/>
            </a>
            <div className={`flex shrink-0 items-center gap-1 ${collapsed ? 'lg:mt-1' : ''}`}>
              <button type="button" onClick={toggleCollapsed}
                className="sanad-focus-ring hidden h-9 w-9 items-center justify-center rounded-lg text-[var(--sanad-text-muted)] hover:bg-[var(--sanad-nav-hover-bg)] lg:flex"
              ...
              <button type="button" onClick={onCloseMobile}
                ...
                aria-label="إغلاق الشريط الجانبي"><X className="h-4 w-4"/></button>
            </div>
          </div>
          <div data-sanad-header-notifications="true"
            className={`mt-0.5 flex min-h-11 items-center justify-between gap-1 ${collapsed ? 'lg:flex-col' : ''}`}>
            <span className={`min-w-0 text-[10px] font-medium leading-4 text-[var(--sanad-text-muted)] ${collapsed ? 'lg:sr-only' : ''}`}>
              مساحة الذكاء والتشغيل
            </span>
            {userId ? (
              <NotificationBell showWorkspaces={false} onNavigate={() => { navigateProduct('notifications'); onCloseMobile(); }} />
            ) : null}
          </div>
        </div>

        <div data-sanad-global-nav-scroll="true"
          className={`min-h-0 flex-1 overflow-y-auto overscroll-contain pb-3 pt-3 [scrollbar-gutter:stable] ${collapsed ? 'px-1 lg:px-1.5' : 'px-3'}`}>
