...
export default function Aurora({
  colorStops = ['#d1fae5', '#dbeafe', '#e0e7ff'],
  speed = 0.22,
  blend = 0.72,
  amplitude = 0.58,
  className = ''
  ...
  useEffect(() => {
    ...
    gl.canvas.className = 'aurora-background__canvas';
    gl.canvas.setAttribute('aria-hidden', 'true');

    const geometry = new Triangle(gl);
    if (geometry.attributes.uv) delete geometry.attributes.uv;

    const toColors = (stops: [string, string, string]) => stops.map((hex) => {
    ...
    const renderFrame = (time: number) => {
      ...
      if (!renderer || !program || !visible || document.hidden) return;

      const current = propsRef.current;
      program.uniforms.uTime.value = reducedMotion ? 0 : time * current.speed * 0.0001;
