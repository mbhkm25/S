...
function statusLabel(value: string): string {
  return ({ queued: 'في الطابور', retry_scheduled: 'إعادة مجدولة', processing: 'قيد المعالجة', completed: 'أُرسلت', failed: 'فشلت', dead_letter: 'توقفت بعد المحاولات' } as Record<string,string>)[value] || value;
}
...
export default function TransactionalMessagingAdmin({ setError, setSuccess }: {
  ...
  const runWorker = async () => {
    ...
    try {
      ...
      setSuccess(`تم تشغيل عامل الرسائل: ${nf.format(result.sent)} مرسلة و${nf.format(result.failed)} غير مكتملة.`);
      await load();
  ...
  return <section className="space-y-3 rounded-[1.7rem] border border-slate-200 bg-slate-50 p-4">
    ...
    {overview && <div className="grid grid-cols-4 gap-2">{[['في الطابور',overview.stats.queued],['قيد المعالجة',overview.stats.processing],['مكتملة',overview.stats.completed],['فشلت',overview.stats.failed]].map(([label,value])=><div key={String(label)} className="rounded-xl bg-white p-3 text-center"><p className="text-sm font-bold">{nf.format(Number(value))}</p><p className="mt-1 text-[8px] text-slate-400">{label}</p></div>)}</div>}
...
function RuleCard({ rule, reload, setError, setSuccess }: {
  ...
  const [working,setWorking]=useState(false);
  const save=async()=>{setWorking(true);try{await updateTransactionalMessageRule({eventType:rule.event_type,enabled,templateName:template.trim(),templateLanguage:language.trim()||'ar',parameterKeys:keys.split('\n').map(x=>x.trim()).filter(Boolean),reason:reason.trim()});setSuccess(`تم حفظ قاعدة: ${rule.display_name}`);setReason('');await reload();}catch{setError('تعذر حفظ القاعدة. تأكد من إعداد الإرسال والسبب الإداري.');}finally{setWorking(false)}};
  const isTemplate=rule.delivery_kind==='template';
  return <article className="rounded-2xl bg-white p-4 shadow-sm"><div className="flex items-start justify-between gap-3"><div><h4 className="text-xs font-bold">{rule.display_name}</h4><p className="mt-1 text-[9px] leading-5 text-slate-500">{rule.description}</p></div><button type="button" onClick={()=>setEnabled(v=>!v)} className={`min-h-9 rounded-xl px-3 text-[9px] font-bold ${enabled?'bg-emerald-50 text-emerald-700':'bg-slate-100 text-slate-500'}`}>{enabled?'مفعّلة':'متوقفة'}</button></div>{isTemplate?<><div className="mt-3 grid grid-cols-[1fr_90px] gap-2"><Field label="اسم قالب Meta"><input dir="ltr" className="admin-input text-left" value={template} onChange={e=>setTemplate(e.target.value.toLowerCase().replace(/[^a-z0-9_]/g,''))}/></Field><Field label="اللغة"><input dir="ltr" className="admin-input text-left" value={language} onChange={e=>setLanguage(e.target.value)}/></Field></div><Field label="ترتيب متغيرات القالب — متغير في كل سطر"><textarea className="admin-input min-h-20 resize-none" value={keys} onChange={e=>setKeys(e.target.value)} /></Field></>:<div className="mt-3 rounded-xl bg-sky-50 p-3 text-[9px] leading-5 text-sky-900"><strong>رسالة خدمية داخل نافذة واتساب:</strong><p className="mt-1 whitespace-pre-wrap">{rule.body_template}</p></div>}<Field label="سبب التعديل الإداري"><textarea className="admin-input min-h-14 resize-none" value={reason} onChange={e=>setReason(e.target.value)} /></Field><button onClick={()=>void save()} disabled={working||reason.trim().length<5||(enabled&&isTemplate&&!template.trim())} className="mt-2 flex min-h-10 w-full items-center justify-center gap-2 rounded-xl bg-slate-950 text-[10px] font-bold text-white disabled:opacity-40">{working?<Loader2 className="h-4 w-4 animate-spin"/>:<><Save className="h-3.5 w-3.5"/>حفظ القاعدة</>}</button></article>;
}
...
function MessageRow({ item, reload, setError, setSuccess }: {
  ...
  const terminalFailure=['failed','dead_letter'].includes(item.status);
  return <article className="rounded-xl bg-white p-3"><div className="flex items-start justify-between gap-3"><div><p className="text-[10px] font-bold">{item.full_name || item.phone_normalized}</p><p className="mt-1 text-[8px] text-slate-400">{item.event_type} · {df.format(new Date(item.created_at))}</p></div><span className={`rounded-lg px-2 py-1 text-[8px] font-bold ${ok?'bg-emerald-50 text-emerald-700':terminalFailure?'bg-rose-50 text-rose-700':'bg-amber-50 text-amber-800'}`}>{statusLabel(item.status)}</span></div>{item.last_error&&<div className="mt-2 flex gap-2 rounded-lg bg-rose-50 p-2 text-[8px] text-rose-700"><AlertTriangle className="h-3 w-3 shrink-0"/>{item.last_error}</div>}{terminalFailure&&<div className="mt-2 grid grid-cols-[1fr_auto] gap-2"><input className="admin-input" value={reason} onChange={e=>setReason(e.target.value)} placeholder="سبب إعادة المحاولة"/><button onClick={()=>void retry()} disabled={working||reason.trim().length<5} className="flex min-h-10 items-center gap-1 rounded-xl bg-sky-50 px-3 text-[8px] font-bold text-sky-700 disabled:opacity-40"><RotateCcw className="h-3 w-3"/>إعادة</button></div>}</article>;
}
