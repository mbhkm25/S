...
function clearIntentParams() {
  const url = new URL(window.location.href);
  for (const key of ['inbox_action', 'inbox_id', 'row_version', 'business_id']) {
    url.searchParams.delete(key);
  }
  window.history.replaceState({}, '', `${url.pathname}${url.search}${url.hash}`);
}

function findOriginalFileButton(): HTMLButtonElement | null {
  const root = document.getElementById('details_view');
  if (!root) return null;
  const buttons = Array.from(root.querySelectorAll('button')) as HTMLButtonElement[];
  return buttons.find(button => button.textContent?.trim().includes('فتح الملف الأصلي')) || null;
}
...
function waitForElement<T extends Element>(resolver: () => T | null, timeoutMs = 15000): Promise<T> {
  return new Promise((resolve, reject) => {
    ...
    const timer = window.setInterval(() => {
      ...
      const accessGate = document.querySelector('[id^="gate_"]');
      if (accessGate || Date.now() - started >= timeoutMs) {
        window.clearInterval(timer);
        reject(new Error(accessGate ? 'access_gate' : 'action_target_timeout'));
      }
    }, 180);
  });
}
...
export default function OperationDetailsActionIntent() {
  ...
  useEffect(() => {
    ...
    const run = async () => {
      ...
      try {
        ...
        const item = {
          id: intent.inboxId || '',
          row_version: Number.isFinite(intent.rowVersion) ? intent.rowVersion : 0,
        ...
        if (intent.action === 'claim_verify') {
          const verifyButton = await waitForElement(() => document.getElementById('btn_confirm_verify') as HTMLButtonElement | null);
          verifyButton.click();
          setMessage('تم استلام العملية وتشغيل إجراء التحقق داخل السجل.');
        } else if (intent.action === 'open_original') {
          const originalButton = await waitForElement(findOriginalFileButton);
          originalButton.click();
          setMessage('تم تشغيل فتح الملف الأصلي من داخل السجل.');
        } else if (intent.action === 'claim') {
          setMessage('تم استلام العملية بنجاح.');
        } else {
          setMessage('تم إكمال العملية بنجاح.');
        }
  ...
  return createPortal(
    <div dir="rtl" className="fixed inset-x-3 bottom-[calc(82px+env(safe-area-inset-bottom))] z-[160] mx-auto flex max-w-md items-center gap-3 rounded-2xl border border-slate-200 bg-white px-4 py-3 shadow-2xl">
      ...
      {status === 'error' && (
        <button type="button" onClick={() => setStatus('idle')} className="text-[10px] font-bold text-slate-500">إغلاق</button>
      )}
    </div>,
    document.body
  );
