...
type Runtime = {
  ...
  timing: {
    ...
    transaction_time_present?: boolean;
    transaction_timezone?: string | null;
    received_at?: string | null;
    received_timezone?: string | null;
  ...
  inbox?: null | {
    ...
    financial_account_holder?: string | null;
    financial_account_verification?: string | null;
    status: string;
    row_version: number;
...
function validRuntime(value: unknown): value is Runtime {
  ...
  return Boolean(
    runtime &&
      runtime.contract_version >= 2 &&
      runtime.read_only === true &&
      runtime.operation?.id &&
      runtime.operation?.public_token &&
      runtime.timing &&
      runtime.document &&
  ...
  return (
    <div className={`rounded-2xl border p-3 ${failed ? 'border-rose-200 bg-rose-50' : 'border-amber-200 bg-amber-50'}`}>
      <div className="flex items-start gap-2.5">
        ...
        <div>
          <p className={`text-[11px] font-black ${failed ? 'text-rose-800' : 'text-amber-900'}`}>
            {failed ? 'تعذر تحليل بيانات الإشعار تلقائيًا' : retrying ? 'تجري إعادة معالجة الإشعار' : 'جارِ تحليل بيانات الإشعار'}
          </p>
          <p className={`mt-1 text-[10px] leading-5 ${failed ? 'text-rose-700' : 'text-amber-700'}`}>
            {failed
              ? 'المستند محفوظ ويمكن فتحه، لكن لا توجد بيانات مالية معتمدة حتى تنجح إعادة المعالجة.'
              : 'يمكنك فتح المستند الآن. ستظهر البيانات المالية فور اكتمال المعالجة دون الحاجة إلى مغادرة الصفحة.'}
          </p>
        </div>
      </div>
    </div>
...
export default function OperationDetailsRuntimeV2() {
  ...
  useEffect(() => {
    const id = window.setInterval(() => setToken((value) => (currentToken() === value ? value : currentToken())), 350);
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      console.error('OperationDetailsRuntimeV2 load failed', cause);
  ...
  useEffect(() => {
    ...
    const id = window.setInterval(async () => {
      try {
        const data = await fetchRuntime();
        if (!cancelled && data) setRuntime(data);
      } catch {
        // Keep the last valid runtime; the next poll may recover.
      }
    }, ANALYSIS_POLL_MS);
    return () => {
      cancelled = true;
  ...
  useEffect(() => {
    ...
    const poll = async () => {
      ...
      if (previewError || data?.status === 'failed' || Date.now() - started > 30000) {
  ...
  const claim = async () => {
    ...
    try {
      await claimPaymentInboxItem({
        ...
        public_token: runtime.operation.public_token,
        business_id: runtime.inbox.business_id,
        status: runtime.inbox.status,
        row_version: runtime.inbox.row_version,
      } as PaymentInboxItem);
      await refresh();
      setMessage('تم استلام العملية وستبقى مسندة إليك حتى إجراء صريح.');
    } catch (cause) {
      setError(cause instanceof Error ? cause.message : 'تعذر استلام العملية.');
  ...
  const complete = async () => {
    ...
    try {
      ...
      setError(cause instanceof Error ? cause.message : 'تعذر اعتماد العملية.');
    } finally { setActing(false); }
  };
  ...
  const personalVerify = async () => {
    ...
    try {
      ...
      await refresh();
      setMessage('تم تسجيل تحققك الشخصي دون تغيير حالة وارد المدفوعات.');
    } catch (cause) {
      setError(cause instanceof Error ? cause.message : 'تعذر تسجيل التحقق.');
    } finally { setActing(false); }
  };
  ...
  const retryPreview = async () => {
    ...
    if (retryError) {
      setPreviewState('failed'); setError(retryError.message); return;
    }
    await supabase.functions.invoke('sanad-operation-preview-worker', { body: {} }).catch(() => null);
  };

  const inboxUrl = useMemo(() => {
    if (!runtime?.inbox) return null;
    const admin = runtime.inbox.status === 'review_required' && runtime.inbox.is_supervisor;
    const query = new URLSearchParams({
      view: admin ? 'payment-inbox-admin' : 'payment-inbox',
      business_id: runtime.inbox.business_id,
      operation_id: runtime.operation.id,
    });
    return `/business/manage/operations?${query}`;
  }, [runtime]);
  ...
  const entity = analysisReady ? runtime.operation.financial_entity || 'جهة مالية أخرى' : 'قيد التحليل';
  const currency = getCurrencyPresentation(runtime.operation.currency);
  const amount = analysisReady && runtime.operation.amount !== null && runtime.operation.amount !== undefined
    ? toLatinDigits(String(runtime.operation.amount))
    : '—';
      ...
      <section dir="rtl" className="mx-auto max-w-2xl space-y-3 pb-36 font-arabic">
        ...
        <article className="rounded-[26px] border border-slate-200 bg-white p-3.5 shadow-sm">
          <div className="grid grid-cols-[48px_minmax(0,1fr)_auto] items-center gap-3">
            ...
            <div className="min-w-0">
              <p className="text-[9px] font-bold text-slate-400">الجهة المالية</p>
              <h1 className="truncate text-[15px] font-black text-slate-950">{entity}</h1>
              {analysisReady && runtime.operation.confidence_score !== null && runtime.operation.confidence_score !== undefined ? <span className="text-[9px] text-slate-400">ثقة الاستخراج {Math.round(Number(runtime.operation.confidence_score) * 100)}%</span> : null}
            </div>
          ...
          {analysisReady ? (
            <div className={`${currentAnalysisState === 'ready' ? '' : 'mt-3'} grid grid-cols-2 gap-x-4 gap-y-2.5`}>
              <Fact label="المستلم" value={runtime.operation.receiver_name || undefined} wide />
              ...
              <Fact label="المرجع" value={runtime.operation.reference_number ? toLatinDigits(String(runtime.operation.reference_number)) : undefined} mono />
              <Fact label="وقت العملية" value={fmt(runtime.timing.transaction_at, 'لم يستخرج')} />
              <Fact label="وصل إلى سند" value={fmt(runtime.timing.received_at)} />
              {lag ? <div className="col-span-2 rounded-xl bg-slate-50 px-3 py-2 text-[10px] font-bold text-slate-600">{lag}</div> : null}
            </div>
          ) : null}
        </article>

        <nav className="grid grid-cols-3 rounded-2xl border border-slate-200 bg-slate-100 p-1">
          {([['operation', 'العملية'], ['document', 'المستند'], ['record', 'السجل']] as Array<[Tab, string]>).map(([value, label]) => (
            <button key={value} onClick={() => setTab(value)} className={`h-10 rounded-xl text-[11px] font-black ${tab === value ? 'bg-white text-slate-950 shadow-sm' : 'text-slate-500'}`}>{label}</button>
          ))}
        </nav>
          ...
          <div className="space-y-3">
            <section className="rounded-3xl border border-slate-200 bg-white p-4 shadow-sm">
              <div className="flex items-start gap-3">
                ...
                <div><h2 className="text-sm font-black text-slate-900">حالة المطابقة والتشغيل</h2><p className="mt-1 text-[11px] leading-6 text-slate-500">فتح الصفحة أو المستند لا يغير حالة العملية. تتغير الحالة فقط عند تنفيذ إجراء صريح.</p></div>
              </div>
              {runtime.inbox ? <div className="mt-3 rounded-2xl bg-slate-50 p-3 text-[11px] leading-6 text-slate-600"><b className="text-slate-900">{runtime.inbox.business_name}</b><br />الحالة الحالية: {statusLabel}{runtime.inbox.claimed_by_name ? ` · المسؤول: ${runtime.inbox.claimed_by_name}` : ''}</div> : <div className="mt-3 rounded-2xl bg-slate-50 p-3 text-[11px] text-slate-600">هذه العملية ليست مرتبطة حاليًا بوارد نشاط يمكنك تشغيله.</div>}
            </section>
        ...
        {tab === 'document' ? (
          <section className="rounded-3xl border border-slate-200 bg-white p-3 shadow-sm">
            <div className="mb-3 flex items-center justify-between px-1"><div><h2 className="text-sm font-black text-slate-900">المستند المالي</h2><p className="text-[9px] text-slate-400">الأصل محفوظ دون تعديل</p></div><span className="text-[9px] font-bold text-slate-400">{previewState === 'ready' ? 'معاينة مركزة' : 'الأصل'}</span></div>
            {documentUrl ? <button onClick={() => setFullscreen(true)} className="relative block w-full overflow-hidden rounded-2xl border border-slate-200 bg-white"><div className="flex min-h-[180px] items-center justify-center bg-white">{previewState === 'ready' || !mime.includes('pdf') ? <img src={documentUrl} alt="معاينة المستند" className="block h-auto max-h-[62dvh] w-full object-contain" /> : <iframe src={documentUrl} title="المستند الأصلي" className="h-[460px] w-full bg-white" />}</div><span className="absolute bottom-3 left-3 inline-flex items-center gap-1.5 rounded-xl bg-slate-950/80 px-3 py-2 text-[10px] font-bold text-white"><Maximize2 className="h-3.5 w-3.5" />تكبير</span></button> : <div className="flex min-h-[220px] items-center justify-center rounded-2xl bg-slate-50"><Loader2 className="h-6 w-6 animate-spin text-slate-400" /></div>}
            {previewState === 'pending' ? <p className="mt-3 text-center text-[10px] text-slate-500">تُجهّز معاينة مركزة، بينما يبقى المستند الأصلي متاحًا.</p> : null}
            {previewState === 'failed' ? <div className="mt-3 flex items-center justify-between gap-2 rounded-2xl bg-slate-50 p-3 text-[10px] text-slate-500"><span>تعذر إعداد المعاينة المحسنة؛ يعرض سند الأصل.</span><button onClick={() => void retryPreview()} className="inline-flex items-center gap-1 font-black text-slate-700"><RefreshCw className="h-3.5 w-3.5" />إعادة المحاولة</button></div> : null}
            ...
            <div className="mt-2 px-1 text-[9px] text-slate-400">{[runtime.document.preview_pipeline_version, fmtBytes(runtime.document.preview_size)].filter(Boolean).join(' · ')}</div>
          </section>
        ) : null}
