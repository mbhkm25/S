      ...
      <div className="flex items-start justify-between gap-3 border-b border-slate-100 bg-gradient-to-l from-indigo-50/80 to-white p-4">
        <div className="flex min-w-0 items-start gap-2.5">
          ...
          <div className="min-w-0">
            <p className="text-sm font-semibold text-slate-950">{review.title || card.title}</p>
            {review.summary ? <p className="mt-1 text-xs leading-5 text-slate-500">{review.summary}</p> : null}
          </div>
        </div>
        <span className={`shrink-0 rounded-full border px-2.5 py-1 text-sm font-semibold ${meta.cls}`}>
          {meta.label}
        </span>
      </div>
      ...
      <div className="space-y-3 p-4">
        <div className="grid gap-2 sm:grid-cols-2">
          {(review.fields || card.fields || []).map((field, index) => (
            <div key={`${field.label}-${index}`} className="border-b border-slate-100 px-1 py-2.5 last:border-b-0">
              <p className="text-sm font-medium text-slate-400">{field.label || 'بيان'}</p>
        ...
        {review.approval_effect ? (
          <div className="flex items-start gap-2 rounded-xl border border-amber-100 bg-amber-50/70 p-3 text-amber-900">
            ...
            <div>
              ...
              <p className="mt-1 text-sm leading-5 opacity-80">{review.approval_effect}</p>
            </div>
          </div>
        ) : null}
        ...
        {review.writes_to_erp ? (
          <div className="flex items-start gap-2 rounded-xl border border-rose-100 bg-rose-50 p-3 text-rose-800">
            ...
            <p className="text-sm leading-5">هذه المسودة تشير إلى كتابة في ERP ولذلك تم تعطيل اعتمادها.</p>
          </div>
        ) : null}
        ...
        {status === 'review' ? (
          <div className="grid grid-cols-3 gap-2">
            <button
              ...
              className="flex min-h-10 items-center justify-center gap-1.5 rounded-xl bg-slate-950 px-2 text-xs font-semibold text-white disabled:opacity-40"
            >
            ...
            <button
              ...
              className="flex min-h-10 items-center justify-center gap-1.5 rounded-xl border border-slate-200 bg-white px-2 text-xs font-semibold text-slate-700 disabled:opacity-40"
            >
            ...
            <button
              ...
              className="flex min-h-10 items-center justify-center gap-1.5 rounded-xl border border-rose-100 bg-rose-50 px-2 text-xs font-semibold text-rose-700 disabled:opacity-40"
            >
